...
def train_full_model(model, name, settings, retrain=False, recall=False):
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_grapheme_root_loss', factor=0.2, patience=2, min_lr=0.0000001, verbose=1)
    early_stopping = EarlyStopping(monitor='val_grapheme_root_loss', patience=10, restore_best_weights=True, verbose=1)
