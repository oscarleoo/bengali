...
def train_full_model(model, name, settings, retrain=False, recall=False):
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_grapheme_root_loss', factor=0.3, patience=3, min_lr=0.0000001, verbose=1)
    ...
    model.compile(optimizer=Adam(settings['learning_rate'], metrics=['categorical_accuracy'], clipnorm=0.1), loss=loss, loss_weights=loss_weights)
    history = model.fit_generator(
        train_generator, steps_per_epoch=train_generator.__len__(), epochs=settings['epochs'],
        validation_data=valid_generator, validation_steps=valid_generator.__len__(),
        callbacks=callbacks
